...
json_file = './trans/metadata_music_abspath.json'
input_dir_wav = './trans/MRSMusic'
output_root = './data/bingrad_music'
fps = 120
...
for item in tqdm(items):
    ...
    wavfile = item['wav_fn'].replace('/home3/panchanghao/data/MRSMusic', './trans/MRSMusic')
    ...
    source_pos = item['source_pos']
    ear_pos = item['ear_pos']
    assert ear_pos[3]==0 and ear_pos[4]==0, '耳朵朝向不对'
    position = np.array([source_pos[0]-ear_pos[0], source_pos[1]-ear_pos[1], source_pos[2]-ear_pos[2], tw, tx, ty, tz])
