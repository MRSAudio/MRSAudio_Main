...
input_dir_wav = 'data/audio_bin/audio_dev/dev-train'
output_root = 'data/bingrad_audio/trainset'
pos_dir = 'data/audio_pos'
fps = 120
rw, rx, ry, rz = euler_to_quaternion(0,0,0)
os.makedirs(output_root, exist_ok=True)
tgt_sr = 48000
wav_files = glob.glob(f'{input_dir_wav}/*.wav')
for wavfile in tqdm(wav_files):
    item = wavfile.split('/')[-1][:-4]
    ...
    pos_fn = os.path.join(pos_dir, f'{item}.npy')
    position = np.load(pos_fn)
    # print(position)
