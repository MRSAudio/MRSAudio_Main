...
def compute_metrics(binauralized, reference, path_pt, path_gt):
    ...
    if isinstance(pesq, np.ndarray):
        pesq = float(pesq.item())  # 处理多元素数组
    else:
        pesq = float(pesq)  # 处理标量
...
ref_folder = 'data/bingrad_audio/testset'
test_sequences = os.listdir(ref_folder)

errors = []
for test_sequence in sorted(test_sequences):
    print(f"Cal {test_sequence}...")
    # binauralize and save output
    binaural, _ = ta.load(f"{ref_folder}/{test_sequence}/binaural_geowarp.wav")

    # compute error metrics
    reference, sr = ta.load(f"{ref_folder}/{test_sequence}/binaural.wav")
    try:
        errors.append(compute_metrics(binaural, reference, f"{ref_folder}/{test_sequence}/binaural_geowarp.wav", f"{ref_folder}/{test_sequence}/binaural.wav"))
    except Exception as e:
        print(f"Error processing {e}")
        continue
    # print(errors[-1])

    if errors[-1]['l2'] * 1000 > 0.5:
        print(errors[-1])
