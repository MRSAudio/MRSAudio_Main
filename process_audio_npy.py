...
def get_position_and_orientation(json_file):
    with open(json_file, "r") as f:
        data = json.load(f)
        position = data["ear_pos"][:3]
        orientation = (data["ear_pos"][3]) % 360
        orientation = np.deg2rad(orientation)  # Convert to radians
...
def process_task(npy_path):
    try:
        seg_number = npy_path.split("_segment_")[-1].split(".npy")[0]
        dir_path = os.path.dirname(npy_path)
        # 查找metadata.json文件
        json_files = [f for f in os.listdir(dir_path) if f.endswith(f"metadata_segment_{seg_number}.json")]
        if not json_files:
        ...
        # print(f"Processing {npy_path} with position {position} and orientation {orientation}")
...
def main():
    base_dir = "./Spatial/MRSAudio/audio"
    
    # 收集所有需要处理的文件路径
    task_paths = []
    for root, _, files in os.walk(base_dir):
        for file in files:
            if file.endswith(".npy") and "segment" in file and "final" not in file:
