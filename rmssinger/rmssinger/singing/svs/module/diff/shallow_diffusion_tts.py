...
class GaussianDiffusionPostnet(GaussianDiffusion):
    def forward(self, cond, ref_mels, coarse_mels, ret, infer, use_cfg=False, scale=2.0, ucond=None):
        ...
        if not infer:
            ...
            # shape = (cond.shape[0], 1, self.mel_bins, cond.shape[2])
            # x = torch.randn(shape, device=device)
            if use_cfg and ucond!=None:
                ucond = ucond.transpose(1, 2)
            for i in tqdm(reversed(range(0, t)), desc='sample time step', total=t):
                if use_cfg and ucond!=None:
                    x = self.p_sample_cfg(x, torch.full((b,), i, device=device, dtype=torch.long), cond, ucond, scale=scale)
                else:
                    x = self.p_sample(x, torch.full((b,), i, device=device, dtype=torch.long), cond)
    ...
    @torch.no_grad()
    def p_sample_cfg(self, x, t, cond, ucond, clip_denoised=True, repeat_noise=False, scale=2.0):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance_cfg(x=x, t=t, cond=cond, ucond=ucond, clip_denoised=clip_denoised, scale=scale)
        noise = noise_like(x.shape, device, repeat_noise)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 1)))
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise
    
    def p_mean_variance_cfg(self, x, t, cond, ucond, clip_denoised: bool, scale=2.0):
        noise_pred_cond = self.denoise_fn(x, t, cond=cond)
        
        noise_pred_ucond = self.denoise_fn(x, t, cond=ucond)
        
        noise_pred= noise_pred_ucond + scale * (noise_pred_cond - noise_pred_ucond)
        x_recon = self.predict_start_from_noise(x, t=t, noise=noise_pred)
